...
SELECT LB.book_id,LB.title, LB.author, LB.genre, LB.publication_year, BC.current_borrowers_count AS current_borrowers
FROM library_books LB
JOIN (
    SELECT book_id, COUNT(record_id) AS current_borrowers_count
    FROM borrowing_records
    WHERE return_date IS NULL
    GROUP BY book_id
) AS BC ON LB.book_id = BC.book_id
WHERE LB.total_copies = BC.current_borrowers_count
