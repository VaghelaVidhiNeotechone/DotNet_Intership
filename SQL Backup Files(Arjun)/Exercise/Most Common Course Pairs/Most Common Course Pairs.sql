...
ordered_courses AS (
    SELECT 
        c.*,
        ROW_NUMBER() OVER (PARTITION BY c.user_id ORDER BY c.completion_date) AS rn
    FROM course_completions c
    INNER JOIN top_performers t ON c.user_id = t.user_id
),
course_pairs AS (
    SELECT 
    ...
    INNER JOIN ordered_courses oc2
